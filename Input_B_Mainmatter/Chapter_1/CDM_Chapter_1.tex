...
For this relation, the equivalence class of $a$ is $\{a\}$. 
The intuition behind the notion of equivalence relation is that an equivalence relation on a set $A$ has the same three properties (reflexivity, symmetry and transitivity) as the equality relation. Intuitively, if $R$ is an equivalence relation on $A$ and $a\in A$, 
then the equivalence class $[a]$ consists of the elements that ``look like they are equal to a'' according to the relation $R$.
Thus, passing from $A$ to the quotient $A/R$, we identify all the elements equivalent to $a$ as a single element of the quotient: $[a]$.
