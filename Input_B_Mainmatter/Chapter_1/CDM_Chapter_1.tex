...
We define a function $f:A\to B$ from $A$ into $B$  as a set of pairs set of ordered pairs $(a,b)$ with $a\in A$ and $b\in B$ (that is, a subset of $A\times B$) 
...
The \emph{range} of $f$ is the set $\{f(a) \colon a\in A \}$.
For each $a\in A$, the unique $b\in B$ such that $(a,b)\in f$ is called the \emph{image} of $a$, and it is denoted $f(a)$.

The set of functions from $A$. into $B$ is denoted $^AB$ or, often, $B^A$.
 
 Given a function $f:A\to B$ , if $A_0$ is a subset of $A$, the \emph{direct image} of $A_0$ is the set 
 \[
 f[A_0]=\{b\in B \colon \exists a\in A_0( f(a)=b) \}.
 \]
  If $B_0$ is a subset of $B$, the \emph{inverse image} of $B_0$ is the set
  \[
 f^{-1}[B_0]= \{a\in A \colon f(a)\in B_0\}.
  \]
  
  Often in the literature, the square brackets ``[''  and ``]'' arereplaced by regular parentheses ``(''  and ``])'. 
  This may lead to confusion in some contexts, so one must be judicious about it, but it is done often for informality.
...
Given a  family $\{A_i \colon i\in I\}$ of sets, the \emph{generalized union} $\bigcup_{i\in I} A_i$ and \emph{generalized intersection} $\bigcap_{i\in I} A_i$ are defined, respectively, as follows:


\begin{align*}
\bigcup_{i\in I} A_i &=\{x \colon \exists i\in I(x\in A_i)\},\\
\bigcap_{i\in I} A_i &=\{x \colon \forall i\in I(x\in A_i)\}.
\end{align*}

\begin{remark}
Note that $\bigcap_{i\in I} A_i $ can only be defined when the index set $I$ is nonempty. If $I$ were empty, then $\bigcap_{i\in I} A_i $ would be a set that contains the universe (the set of all sets), which does exist in axiomatic set theory (due to Russell's paradox).
\end{remark}


    
 \begin{exercise}
 Let $f:A\to B$ be given and let $\{A_i \colon i\in I\}, \{B_i \colon i\in I\}$ be indexed families of subsets of $A,B$ respectively. Determine if the following assertions are true or false:
 \begin{enumerate}
 \item
 ...
 $f^{-1}[\bigcap_{i\in I}B_i] =  \bigcap_{i\in I} f^{-1}[B_i]$.
      \stepcounter{enumi}
 \item[(\theenumi)*] 
 For the parts above where your answer was ``false'', determine the exact condition on $f$ that is needed to make the answer ``true'' for arbitrary indexed families $\{A_i \colon i\in I\}, \{B_i \colon i\in I\}$  of subsets of $A,B$ respectively.
...
\begin{enumerate}
\item
An \emph{equivalence relation on $A$}  is a binary relation on $A$ that is reflexive, symmetric, and transitive.
\item
$R$ is an equivalence relation on $A$ and $a\in A$, the \emph{equivalence class of $a$} is the set
\[
[a]=\{ x\in A :  xRa\}.
\]
\end{enumerate}
The $\{[a] : a\in A\}$ of equivalence classes of $R$ is denoted $A/R$, and it is called the \emph{quotient} of $A$ by $R$.
\end{definition}

The motivating example of an equivalence relation is the equality relation $=$ (on any set). 
For this relation, the equivalence class of $a$ has only one element:~$a$. 
The intuition behind the notion of equivalence relation is that an equivalence relation on a set $A$ has the three distinctive properties (reflexivity, symmetry and transitivity) as the equality relation without necessarily being the equality relation.
Intuitively, if $R$ is an equivalence relation on $A$ and $a\in A$, 
then the equivalence class $[a]$ consists of the elements that would be equal to a if  $R$ were true equality.
By passing from $A$ to the quotient $A/R$, we identify all the elements equivalent to $a$ as a single element of the quotient: $[a]$. In standard mathematical practice, we pass to the quotient $A/R$ whenever we are in contexts where want all elements of $A$ that are equivalent to $a$ through $R$ to be regarded as the same element.


\begin{definition}
\label{D:part}
Let $A$ be a nonempty set. A \emph{partition} of $A$ is an indexed family 
\[
\{A_i : i\in I\}
\]
of subsets of $A$ such that for every pair of indices $i,j\in I$, we have:
\begin{enumerate}
\item
$A_i\neq \emptyset$,
\item
...
\begin{exercise}
Prove that if $R$ is an equivalence relation on a nonempty set $A$, then the equivalence classes of $R$ form a partition of the set $A$.
\end{exercise}
...
\begin{exercise}
Prove that an infinite set is countable if and only if if there exists a bijective function $f:\BbN \to A$. 
\end{exercise}
