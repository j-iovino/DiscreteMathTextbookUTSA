...
In the same way that we defined ordered pairs we can define ordered triples, quadruples, quintuples, and, in genral $n$ tupkles $(a_1,\dots, a_n)$ for any nonnegative integer $n$.
If $A$ is a set, the set of $n$-tuples $(a_1,\dots, a_n)$ with $a_1,\dots, a_n\in A$ is denoted $A^n$.

For this course, we will assume that participants are familiar with the notion of function, the arrow notation $f\colon A\to B$, and the concepts of injective and surjective functions. 
Nevertheless, we summarize it here. If $,B$ are sets, a function $f:\to B$ is intuitively a ``rule'' that to every $a\in A$ assigns a unique $b\in B$. 
The notions of ``rule'' and ``assign'' are not mathematically precise, but we con formalize them within the framework of set theory as follows.
A function $f:A\to B$ is a set of pairs set of ordered pairs $(a,b)$ with $a\in A$ and $b\in B$ (that is, a subset of $A\times B$) 
For every $a\in A$ there exists  a unique $b\in B$ such that $(a,b)\in f$, i.e.,
\[
\forall a (\exists b\in B ((a,b) \in f) \land (\forall b',b'' ((a,b')\in f \land (a,b'')\in f) \to b'=b'')).
\]
In this case, we write $f: A\to B$. 
...
For each $a\in A$, the unique $b\in B$ such that $(a,b)\in f$ is called the \emph{image} of $a$, and it is denoted $f(a)$.
 ...
 Given a function If $A_0$ is a subset of $A$, the \emph{direct image} of $A_0$, denoted $f[A]$,  is the set $\{b\in B \mid \exists a\in A_0( f(a)=b) \}$. 
  ...
  \[
  \forall b \in B \exists  ((a,b)\in f).
  \]
  
  
 A function $f: A\to B$ is \emph{injective} if distinct points in $A$ have distinct images in $B$; symbollically
   \[
  \forall a, a'\in A  (a \neq a' \to f(a)\neq f(a')).
    \]

This definition is often written in contrapositive form:
   \[
