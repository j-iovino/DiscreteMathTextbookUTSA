...
By passing from $A$ to the quotient $A/R$, we identify all the elements equivalent to $a$ as a single element of the quotient: $[a]$. In practice, we pass to the quotient $A/R$ when we want all elements of $A$ that are equivalent to $a$ through $R$ to be regarded as the same element.


\begin{definition}
\label{D:part}
Let $A$ be a set. A \emph{partition} of $A$ is an indexed family 
\[
\mathcal{P}=\{A_i : i\in I\}
\]
of subsets of $A$ such that for every pair of indices $i,j\in I$, we have 
\[
\text{either $A_i=A_j$ or $A_i\cap A_j=\emptyset$}.
\]
\end{definition}

\begin{exercise}
Prove that if $R$ is an equivalence relation on a set $A$, then the equivalence classes of $R$ form a partition of the set $A$.
\end{exercise}
